...
//checks if lef and right are null. True if they are false otherwise
...
AVLTree::AVLNode* AVLTree::insertPart2(AVLNode* parent, const string& key, size_t valueOfKey) {
    ...
    if (parent == nullptr) {
        //makes the new node with the key and value
        AVLNode* rootNode = new AVLNode(key, valueOfKey);
        //sets the left, right, and parent to nullptr before inserting the new node as root
    ...
    if (key < parent->key) {
        //makes a new node and calls insertPart2 to recursively call it
        AVLNode* childNodeLeft = insertPart2(parent->left, key, valueOfKey);
        //sets the child on the left
        AVLTreeSetChild(parent, "left", childNodeLeft);
    }
    //checks down right path
    else {
        //makes new right node with insertPart2
        AVLNode* childNodeRight = insertPart2(parent->right, key, valueOfKey);
        AVLTreeSetChild(parent, "right", childNodeRight);
    }
